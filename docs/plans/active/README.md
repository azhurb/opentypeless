...
- [Gemini Transcribe](gemini-transcribe.md) — the batch `gemini-3.5-transcribe` provider is landed but has never run against a real key; lists the inferred parts of the wire shape and the deferred items (the `-live` streaming provider, vocabulary biasing for the other providers).
