...
**What that costs — much less than first claimed.** When the fallback was removed this section said Accessibility could not reach browser web content or Electron. That was wrong, and wrong for a reason worth recording: the read was asking the **system-wide** element, which on macOS 26 (Darwin 25.5) answers `kAXErrorCannotComplete` in 0 ms for *every* app, TextEdit included. Native apps only appeared to work because the Cmd+C fallback was covering for a read that had already failed. Asking the frontmost application's own element instead succeeds in ~31–53 ms and returns the selection — in TextEdit and in a Gmail draft in Chrome alike. See `focus_root` in `correction::ax_macos`.

What genuinely does not work: **Monaco-based editors** (VS Code, Cursor), which answer promptly with `kAXErrorNoValue` — they publish no focused element until a screen reader is detected — and **every non-macOS platform**, since `correction::ax_stub` has no implementation. The Settings toggle is disabled on Windows and Linux for the same reason it is disabled when polish is off: a switch that can never fire is worse than an absent one. Both remaining gaps are tracked in [`docs/plans/active/selected-text-in-ax-blind-apps.md`](../plans/active/selected-text-in-ax-blind-apps.md).
